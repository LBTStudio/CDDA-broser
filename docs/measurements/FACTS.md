...
2. **Asyncify 計装ありの wasm と無しの wasm で CPU 速度が変わらない**
   ...
   > **⚠️ 2026-09-08 追記（F-29 参照）: この 0.1030 / 0.1250 は
   > CDDA のターン処理の実測ではない。**
   > 出典 `bench/asyncify_overhead.cpp` は仮想呼び出しを模した
   > 合成コードである。「計装の有無で差が出ない」という**比較の結論は
   > 有効**（同一コードの A/B なので）だが、
   > **0.1030 を「CDDA の 1 ターンのコスト」として引用してはいけない。**
...
> **⚠️ 2026-09-08 追記（F-29 参照）: 上表は実測値ではない。**
> 出典 `bench/activity_paths.c` の中身は
> `static void burn( int units ) { for( i<units ) s += sqrt( i & 1023 ); }`
> という空ループで、`part_mon_info() { burn( 60000 ); }` の 60000 は
> **手で決めた定数**である。実際の `mon_info_update()` を測ったものではない。
> F-01 の 0.1030 ms/turn（`bench/asyncify_overhead.cpp`）も同様に合成値で、
> **両者は 8.3 倍食い違っている**。
> 各部品の**相対的な重さの目安**としては参考になるが、
> **絶対値を根拠に最適化の優先順位を決めてはいけない。**
> 実測は `patches/cdda-0I-emscripten-activity-perf.patch` が
> 追加した `[perf]` ログで取ること。
...
## F-28. 「移動が追いつかない」の真因は入力の取りこぼしだった（2026-09-07）

- **確定日**: 2026-09-07
- **根拠**: `raw/2026-09-07-input-queue-drop.log`
  + 一次資料 `src/sdltiles.cpp`（`CheckMessages` / `pump_events` / `get_input_event`）
  + 本番配信物 `https://lbtstudio.github.io/CDDA-broser/cataclysm-tiles.js`
- **実装**: `patches/cdda-0I-emscripten-input-queue.patch`
- **ベンチ**: `bench/input_queue_drop.c` / `bench/input_queue_pump_discard.c`
  / `bench/input_queue_fix.c`

### F-28-0. 症状

> 「マップを移動している感じ、反応というか追従性がやはり悪い。
>   **短時間に何度も移動すると追いつかなくなる**。これをリアルタイム性にしてほしい」

### F-28-1.【最初にやったこと】yield が原因でないことを配信物で確定

F-01 で `emscripten_sleep(0)` 4.25ms → MessageChannel 0.075ms（57 倍）の
改善を入れているが、**それが本当に本番で動いているか**を先に確認した。

本番 JS（342,536 B）を実際にダウンロードして中身を確認:

```js
function __asyncjs__cata_web_yield_impl(){return Asyncify.handleAsync(async()=>{
  if(!Module._cataWebYieldChannel){var ch=new MessageChannel; ... }
  await new Promise(function(resolve){Module._cataWebYieldChannel.push(resolve)})})}
```

**MessageChannel 版 yield は稼働している。** よって残る症状は
yield の 1 回あたりコストとは別の原因である。

この確認をしなければ、また yield の改善に時間を使っていた。

### F-28-2.【真因】押したキーが実際に捨てられていた

`CheckMessages()` は 1 回の呼び出しで SDL キューを**最後まで飲み干す**。
波括弧の対応で機械的に確定させた:

```
loop starts at line 3166
loop ends at line 3742  total lines: 577
★ ループ自体を抜ける break/continue:
   line 3740: break;   文脈: if( text_refresh && !is_repeat ) {
```

抜ける経路は `SDL_TEXTINPUT` 用の 1 個だけ。しかも移動は
`input_context( "DEFAULTMODE", keyboard_mode::keycode )`（`game.cpp:2676`）
なので `SDL_TEXTINPUT` を経由せず、この break は**発火しない**。

受け皿は `last_input`（`sdltiles.cpp:150`）という**単一変数**なので、
飲んだ N 個は全て上書きされ、呼び出し側に渡るのは**最後の 1 個だけ**。
...
という構造で、これが **src 内 35 箇所**から呼ばれる。
移動 1 回のターン処理でも `do_turn.cpp:306` を通る。

### F-28-3. なぜ「短時間に何度も」で顕著になるのか

| 押し方 | ブラウザの配送 | 結果 |
|---|---|---|
| ゆっくり 1 回ずつ | 1 個ずつ届く | 取りこぼしゼロ |
| **短時間に何度も** | yield の瞬間に**まとめて**届く | **最後の 1 個以外全部消滅** |

wasm がメインスレッドを占有している間、ブラウザは keydown を
DOM 側に溜め込み、yield した瞬間にまとめて配送する。

**「追いつかない」のではなく「入力が消えている」。**

### F-28-4. 実測（修正前）

| 計測 | 結果 |
|---|---|
| 12 回押す（pump なし） | pump が見たのは 12、**実際に使われたのは 1**、消滅 11 |
| ターン処理中に 10 回押す | `get_input_event()` が受け取れたのは **0** |

### F-28-5. 対策と効果

単一変数 `last_input` を FIFO キュー（上限 64）で補完する。

| 押した回数 | 修正前（ターン処理中） | 修正後 | 修正前（まとめて配送） | 修正後 |
|---|---|---|---|---|
| 1 | 0 | **1** | 1 | **1** |
| 2 | 0 | **2** | 1 | **2** |
| 3 | 0 | **3** | 1 | **3** |
...
**押した回数と処理される回数が完全一致するようになった。**

### F-28-6. 設計判断（なぜこの実装にしたか）

| 判断 | 理由 |
|---|---|
| 退避は**排出ループの冒頭**で行う | switch は 577 行あり `last_input` 代入点が十数箇所に散在。各点に手を入れると漏れるし上流の追加に追従できない。「次の周回に入る直前」の 1 点なら全部拾える |
| ループを抜けた後の `last_input` は退避**しない** | そのまま呼び出し側へ渡るので、入力 1 個の通常時は**従来と完全に同一**の経路になる |
| `pump_events()` は破棄をやめて**キューへ戻す** | 上流の意図は「このターンではキー入力として扱わない」で「捨てる」ではない。ネイティブは OS キューに残るので実害なし、ブラウザは SDL キューを空にした時点で唯一の在処が消える |
| `get_input_event()` はキューを**最優先**で返す | 溜まっている＝操作が未消化。ここで描画や yield を挟むと捌く速度が rAF（15.9ms）に縛られる |
| `drain_pending_input_if_idle()` を **3 分岐すべて**に | 排出ループ最後のイベントが `last_input` を設定しない種類（無視マウス移動・ウィンドウイベント・未対応キー）だと error のままキューに残る |
| マウス移動は**溜めない** | 「最新位置」以外に意味がなく、溜めると移動キーの後ろに並んで実移動が遅れる |
| 上限 **64** で古いものから捨てる | 押しっぱなし・タブ復帰後に無制限に溜まると指を離した後も動き続ける。64 = 60fps で 1 秒ぶん |

### F-28-7. ゲームプレイ体験を損なわないことの根拠

- キューが空の**通常時は全分岐を素通り**するので、従来の挙動・
  描画タイミングと完全に同一。複数溜まったときだけ働く。
- ネイティブでは OS のイベントループが別に回っており、1 回の
  `CheckMessages()` に複数キーが同時に溜まることは実質ない。
  **このキューは「ネイティブでは自然に成立していた 1 入力 = 1 ターン」を
  ブラウザで復元するもの**であり、ゲームの意味を変えない。
  押した回数だけ動くのが本来の CDDA の挙動である。
- 非 EMSCRIPTEN は `#else` で全て空実装。ネイティブは 1 命令も変わらない。

### F-28-8.【重要】検証機構そのものを negative test で検証した

`ci/apply-patches.sh` に検査を追加したが、**「検証が通った」だけでは
検証が機能しているか分からない**ので、意図的に壊して確認した。

| 壊し方 | 結果 |
|---|---|
| `stash_pending_input()` をコメントアウト | rc=1（検出成功） |
| `pump_events()` の退避を削除 | rc=1（検出成功） |
| `drain_pending_input_if_idle()` を 1 箇所削る | rc=1（検出成功） |
| 復元後 | `[VERIFY] OK: 全 8 パッチが意図どおり適用された` |

**最初の実装では grep が部分一致だったためコメントアウトを見逃していた**
（rc=0 で通ってしまった）。行頭一致 `'^[[:space:]]*...'` に直した。

また `stash` の位置検査は最初 `grep -B 3` で書いたが、間に説明コメントが
20 行入るため機能しなかった。行数固定をやめ、行番号の大小比較

```
排出ループ開始行 < stash 行 < imclient->process_input 行
```

に変えた。上流がループ構造を変えたときだけ失敗する。

### F-28-9. 一般化できる教訓

1. **「遅い」という症状は、必ずしも「処理が重い」ではない。**
   今回は「入力が消えていた」。体感は同じでも原因が違うので、
   処理の高速化をいくら進めても直らなかった。
   症状の言葉（追いつかない）を、そのまま原因（処理が遅い）と
   読み替えてはいけない。

2. **既に施した最適化が効いているかを、まず配信物で確認する。**
   本番 JS に MessageChannel 版 yield が入っていることを確認したから
   こそ「yield ではない別の原因がある」と判断できた。

3. **ネイティブでは無害な実装が、ブラウザでは致命的になる。**
   `pump_events()` の `last_input` 破棄は、OS のイベントキューが別に
   存在するネイティブでは実害がない。「SDL キューを空にしたら、
   それが唯一の在処になる」という前提の違いが欠陥に変わる。

4. **制御フローは目視ではなく機械的に確定させる。**
   577 行の switch に対して「break があるか」を目で追うのは信頼できない。
   波括弧対応で切り出し、switch 内 break を除外して初めて
   「抜ける経路は 1 個だけ」と確定できた。

5. **検証コードは必ず negative test を通す。**
   grep の部分一致でコメントアウトを見逃していた。
   「壊したときに失敗すること」を確認しない検証は、
   検証していないのと同じである。

---

## F-29. 【重要】これまでの「1 ターンのコスト」は実測ではなかった。実測基盤を入れた（2026-09-08）

- **確定日**: 2026-09-08
- **根拠**: `raw/2026-09-08-activity-perf-instrumentation.log`
  + 一次資料 `bench/activity_paths.c` / `bench/asyncify_overhead.cpp`
  + `src/do_turn.cpp` / `src/activity_actor.cpp`
- **実装**: `patches/cdda-0I-emscripten-activity-perf.patch`
- **ベンチ**: `bench/activity_profiler_logic.cpp` / `bench/progress_refresh_rate.c`
...
> 「個人的に**リアルタイム基準の進行にするのは反対**です。
>   ターン基準だからこそゲームに即していると考えられます。
>   ただそれを実処理時間として考えると長すぎる上に
>   **どの程度作業に時間を要するかが判然としない**場合が
>   あったりするのがネックなのです。
>   **本家ＣＤＤＡと同等の処理時間で収める**必要があります」

### F-29-1.【最重要】根拠にしていた数値が 2 つとも合成値だった

「1 ターンの実処理時間」として本プロジェクトが根拠にしてきた数値:

| 出典 | 値 | 実体 |
|---|---|---|
| F-18 | 0.8600 ms/turn | `bench/activity_paths.c`。`burn(60000)` の **60000 は手で決めた定数** |
| F-01 | 0.1030 ms/turn | `bench/asyncify_overhead.cpp`。仮想呼び出しを模した**合成コード** |

`bench/activity_paths.c` の実体:
...
static void part_mon_info( void )  { burn( 60000 ); }  /* ← 実コードではない */
static void part_monmove( void )   { burn( 40000 ); }
```

**この 2 つは 8.3 倍食い違っている**（0.8600 / 0.1030 = 8.35）。
どちらも実プレイの計測ではないため、
**「本家 CDDA と同等の処理時間に収める」という目標に対して
出発点となる数値が存在しない状態**だった。

これは本プロジェクトが繰り返してきた失敗の型そのものである:

| 前例 | 誤り | 実際 |
|---|---|---|
| F-23 | リンクが遅いのは Asyncify | `inlining-optimizing` パス |
| F-27 | `em++` の exit code 0 = 成功 | 起動不能な wasm を配信 |

### F-29-2.【却下】進捗表示を実時間基準にする案
...
| **それ以外**（粉砕・読書・製作・解体） | `5_minutes` = **300 ターン** |

これを実時間 200ms 基準に変える実装を書いたが**撤回した**。

1. **ユーザーの明確な方針**: 「ターン基準だからこそゲームに即している」
2. **より本質的な問題**: 実時間基準にすれば表示は滑らかになるが、
   それは「実処理時間が本家より長い」という**本質に蓋をするだけ**。
   直すべきは 1 ターンあたりの実処理コストそのものである。
3. さらに、試算の前提が F-29-1 の**信用できない 0.8600 ms** だった。

撤回は `due_by_real_time` / `last_wait_redraw_ms` /
`wait_redraw_interval_ms` の grep が **0 件**であることで確認した。

### F-29-3. 採った方針: まず実測できる状態を作る

`src/do_turn.cpp` に `sample_activity_turn()` を追加。
アクティビティ（睡眠含む）の継続中に**経過ターン数・実経過時間**を
集計し、終了時に 1 行だけ報告する。
...
これは**報告の 2 つの不満に同時に答える**:

| 不満 | どう答えるか |
|---|---|
| 「本家と同等の処理時間で収める必要がある」 | プレイヤー環境での実値が出る。本家で同じ行動をして比べれば達成度が客観的に判定できる |
| 「どの程度時間を要するか判然としない」 | 行動終了時に「何ターンで何 ms」が必ず残る |

### F-29-4. 設計判断

| 判断 | 理由 |
|---|---|
| **常時有効**（デバッグ限定にしない） | 有効化を頼む往復が発生する。コストは `steady_clock` 1 回と整数加算だけ。報告は終了時 1 行なのでログを汚さない |
| 1 ターンで終わる行動は報告しない | 役に立たずログを埋めるだけ |
| 切り替わったら前の分を報告して測り直す | 捨てると連続作業（解体→粉砕）の内訳が分からない |
| **`do_turn.cpp` の匿名 namespace に置く** | 下記参照 |

**なぜ `game.h`/`game.cpp` ではないのか（重要）**:
最初 `game.h` にクラスを宣言し `game.cpp` に実装したが撤回した。
`game.cpp` は約 14000 行で巨大なヘッダ群を含み、本サンドボックス
（**メモリ 985MB / 2 コア**）では `em++ -fsyntax-only` すら完走しない。
実際に 120 秒でタイムアウトし、バックグラウンドで 6 分以上回しても
終わらず、最終的に**サンドボックス全体が応答不能になり
`ResetSandbox` が必要**になった。

**検証できない変更を入れるのは F-27 の再来**なので、
呼び出し元と同じ翻訳単位に閉じて必ず検証できる形にした。

### F-29-5. 併せて直した: 粉砕に進捗表示が無かった

これは数値の不確かさとは**独立した構造的欠陥**である。

`pulp_activity_actor` には `get_progress_message()` が**未実装**で、
`activity_actor` の既定実装（空文字列）が使われていた。
そのため画面には verb だけの `Smashing…` しか出ず、
**残り死体数も現在の死体の進捗も分からない**。

なぜブラウザで特に効くか:
`pulp_activity_actor::do_turn` は「1 ターンで死体 1 個だけ」叩く
（`activity_actor.cpp:8762`）。1 死体を壊すには `time_to_pulp` 回
必要で、人間サイズ（62.5L）で最低 **30 ターン**
（`game.cpp:14345` `min_time_to_pulp = 30 * corpse_volume / 62.5`）。
**死体 20 個なら 600 ターン以上**になる。

| 状況 | 表示 |
|---|---|
| 複数残っている | `3 corpses left (45%)` |
| 最後の 1 体 | `45%` |

走査順を `do_turn` と同じ（後ろから）にしている。順序が違うと
「進捗が進んでいるのに数字が動かない」という別種の分かりにくさを生む。

### F-29-6. ターン基準を維持したうえで追加した yield

更新間隔の判定は**ターン基準のまま**だが、
「ターン基準で描くと決まった回に、その内容が実際に画面へ出る」
ようにする yield を 1 つ追加した。
...
これは**表示頻度を変えるものではない**（回数はターン基準のまま）。

### F-29-7.【重要】negative test で自分の検証の欠陥を 2 件見つけた

`ci/apply-patches.sh` に検査を追加したが、意図的に壊して確認した。

| 壊し方 | 結果 |
|---|---|
| 計測呼び出しをコメントアウト | rc=1（検出成功） |
| 粉砕の実装を削除 | rc=1（検出成功） |
| pulp クラス内の宣言だけ削除 | rc=1（検出成功） |
| 呼び出しを 2 重にする | rc=1（検出成功） |
| 計測を `do_turn` の直列部分の外へ移動 | rc=1（検出成功） |
| 復元後 | `[VERIFY] OK: 全 9 パッチが意図どおり適用された` |

**この過程で踏んだ 2 つの見逃し**:

1. **テスト harness に `set -e` が無かった**
   検証ブロックだけを `sed` で抽出して実行したが、本番スクリプト冒頭の
   `set -euo pipefail` を含めていなかった。そのため `grep -q` の失敗が
   無視され、**壊したのに rc=0** になっていた。

2. **宣言の grep をファイル全体にかけていた**
   `std::string get_progress_message( const player_activity & ) const override;`
   という同一の行は他の actor にも存在し、**実測でファイル内に 4 箇所**あった。
   pulp のものだけ消してもファイル全体の grep は通り、**検出できなかった**。
   → pulp クラスの本体だけを切り出して検査する形に修正した。

いずれも「検証は通ったのに機能していない」という最悪の失敗モードで、
**negative test をしなければそのまま出荷していた**。

### F-29-8. 位置検査の設計

計測呼び出しが `do_turn()` の**直列部分**にあることを行番号で検査する:

```
debug_hour_timer.print_time() 行 < sample_activity_turn 行 < u.update_body() 行
```

この 3 つは直列部分に並んでいるので、順序が保たれていれば
ループの外にあることが保証される。**ループ内に入ると 1 ターンで
複数回加算され計測が壊れる**ため必須。
行数固定の `grep -A/-B` では検査できない（間に説明コメントが 60 行以上入る）。

### F-29-9. 次にやること（この順序を守ること）

本パッチをデプロイし、プレイヤーの実環境で `[perf]` の
`X ms/turn` を得る。その値とネイティブ本家 CDDA の同一行動の値を
比較して初めて「本家同等」への距離が分かる。

候補（**実測後に**優先順位を決める）:
`mon_info_update()` の LOS 全走査 / `build_map_cache` /
`build_floor_caches` の層数 / `process_items` / `monmove` /
Asyncify 計装。

**合成ベンチの数字で優先順位を決めると F-23 と同じ失敗を繰り返す。**

### F-29-10. 一般化できる教訓

1. **ベンチマークが「実物を測っているか」を必ず確認する。**
   `burn(60000)` のような合成負荷は、書いた本人には「モデル化」に
   見えるが、後から読むと実測値と区別できない。FACTS.md に載った
   時点で誰も疑わなくなる。**合成なら合成とその場に明記する。**

2. **数字が 2 つあって食い違っているなら、両方疑う。**
   8.3 倍の差に気付いた時点で「どちらかが正しい」ではなく
   「どちらも怪しい」と考えるべきだった。

3. **ユーザーの方針は最適化の前提条件として扱う。**
   「ターン基準だからこそゲームに即している」は技術的な良し悪しの
   話ではなく**設計の前提**である。実時間基準案は技術的には
   成立していたが、前提に反するので却下するのが正しい。

4. **検証できない場所に変更を入れない。**
   `game.cpp` に実装を置いたままなら、構文検証すらできない状態で
   コミットしていた。それは F-27 と同じ構図である。
   検証できる場所へ実装を移すのは妥協ではなく**必須条件**。

5. **negative test は harness ごと疑う。**
   `set -e` の欠落と grep の範囲、2 件とも harness 側の欠陥だった。
   「壊したのに通った」を見たとき、まず疑うのは harness である。
