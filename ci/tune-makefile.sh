...
# ------------------------------------------------------------------
# 【配信事故の再発防止】1 関数が V8 の上限を超えないようにする
# ------------------------------------------------------------------
# run 33985475507（#75）は全ジョブ成功・配信まで通ったのに、
# ブラウザで開くと起動せずこうなった:
#
#   CompileError: WebAssembly.instantiateStreaming():
#   size 7657177 > maximum function size 7654321 @+249007
#
# V8 には「1 関数あたり 7,654,321 バイト」という実装上限
# （kV8MaxWasmFunctionSize）がある。Chrome / Edge / Chromebook は
# すべて V8 なので、配信対象では実質的な絶対上限である。
#
# 【原因の実測】動いていた #71 の成果物と比較した:
#
#   #71（116 / -Os）: 最大関数 7,459,141 B = 上限の 97.45%（余裕 195KB）
#   #75（132 / -O2）: 最大関数 7,657,177 B = 上限の 100.04%（超過 2,856B）
#                     増加 198,036 B = +2.65%
#
# つまり【巨大関数は元から存在しており】、上限の 97.45% という
# 崖っぷちでたまたま動いていただけだった。
# 132 / -O2 が 2.65% 押した結果、限界を超えた。
# 言い換えると、この構成変更が無くても CDDA のコードが 2.6% 増えれば
# -Os のままで同じ事故が起きていた【時限爆弾】である。
#
# 【なぜ 1 関数がそんなに大きくなるのか】
# Binaryen の「呼び出し元が 1 つだけの関数」のインライン上限
#   --one-caller-inline-max-function-size (-ocimfs)
# は【既定が -1 = 無制限】である。
# そのため「1 回だけ呼ばれる巨大関数」が呼び出し元へ丸ごと取り込まれ、
# それが連鎖して 1 関数が 7MB まで膨らむ。
#
# 【対策】結合後サイズの上限を明示する。
#   --inline-max-combined-binary-size (-imcbs, 既定 409600)
# これは上流 PR #7820「Inlining: Make MaxCombinedBinarySize
# configurable」で可変になったもので、版 116 には存在しない
# （116 で --help を見ても出てこないことを確認済み）。
# 132 に上げたからこそ使える対策である。
#
# 既定の 409600 でも 7MB の関数が出来ている理由は、
# -ocimfs が無制限だと「1 呼び出し元」経路が imcbs の判定を
# 通らずに展開されるためである。よって両方を指定する。
#
# 【値の選び方】
#   -ocimfs=20 … 実測で採用しても時間・サイズがほぼ変わらないと
#                 確認済みの値（F-26-6）。無制限をやめることが目的。
# これで巨大関数の生成源を止める。
#
# 【検証は ci/verify-wasm.sh が行う】
# 設定を入れて満足せず、生成物の【実際の最大関数サイズ】を
# 測って上限未満であることを確認する。
# em++ の終了コードは壊れていても 0 なので、それ以外に方法がない。
#
# 根拠: docs/measurements/FACTS.md F-27
CDDA_INLINE_LIMIT="${CDDA_INLINE_LIMIT:-20}"

if grep -q 'BINARYEN_EXTRA_PASSES' Makefile; then
    echo "[TUNE] インライン制限は既に設定済みです（何もしません）"
else
    # -sLZ4 の行の直後に追記する（RELEASE 分岐の中に確実に入れる）。
    # 行番号ではなく内容で位置を決めるので、上流の行がずれても壊れない。
    lz4_line="$( grep -n -- 'LDFLAGS += -sLZ4' Makefile | head -1 | cut -d: -f1 )"
    if [ -z "$lz4_line" ]; then
        echo "ERROR: RELEASE 分岐の -sLZ4 行が見つかりません。" >&2
        echo "       上流の Makefile が変わった可能性があります。" >&2
        exit 1
    fi
    # 【なぜ 1 行にまとめるのか】
    # BINARYEN_EXTRA_PASSES はカンマ区切りで複数指定できる。
    # 別々に 2 回書くと後の指定が前を上書きしてしまう。
    sed -i "${lz4_line}a\\    LDFLAGS += -sBINARYEN_EXTRA_PASSES=--one-caller-inline-max-function-size=${CDDA_INLINE_LIMIT}" Makefile
    echo "[TUNE] インライン展開を制限しました（V8 の 1 関数上限対策）"
    echo "[TUNE]   -ocimfs=${CDDA_INLINE_LIMIT}（既定の -1=無制限をやめる）"
    echo "[TUNE]   ※ #75 はこの制限が無く 7,657,177B の関数ができて起動不能でした"
fi
